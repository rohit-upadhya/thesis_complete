...
def filter_results(results):
    ...
    for result in results:
        ...
        if size >= 12.0 or "Calibri-Bold" in font or link is not None or "pct." in text:
...
def combine_entries_with_section(results):
    ...
    while i < len(results):
        text, size, font, link = results[i]
        if link is not None and i + 1 < len(results) and "pct." in results[i + 1][0] and results[i + 1][3] is None:
...
if __name__ == "__main__":
    ...
    for file in files :
        ...
        text_file_output =  os.path.join("output","romanian","results2.txt")           
