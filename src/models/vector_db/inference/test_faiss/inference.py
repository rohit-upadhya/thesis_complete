...
class Inference:
    ...
    def main(self):
        ...
        recall_data = {}
        with open(os.path.join('output/inference_outputs', f'recall_multi.json'), 'r') as json_file:
            recall_data = json.load(json_file)
    
        recall_data[self.file_base_name] = recall
        with open(os.path.join('output/inference_outputs', f'recall_multi.json'), 'w+') as json_file:
