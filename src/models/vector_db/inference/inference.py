...
class Inference:
    ...
    def main(self):
        ...
        recall_data = {}
        with open(os.path.join('output/inference_outputs',f'recall_one_db_mbert.json'), 'r') as json_file:
            recall_data = json.load(json_file)
    
        recall_data[self.file_base_name] = recall
        with open(os.path.join('output/inference_outputs',f'recall_one_db_mbert.json'), 'w+') as json_file:
            json.dump(recall_data, json_file, indent=4, ensure_ascii=False)
        return results

if __name__ == "__main__":
    languages = ["english", "french", "italian", "romanian", "russian", "turkish",]
    languages = ["romanian", "russian", "turkish","ukrainian"]
