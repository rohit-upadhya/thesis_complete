...
class ParagraphGATAllGating(nn.Module):
    ...
    def mix_actual_nodes(self, x1, x2, num_actual_nodes1, num_actual_nodes2):
        
        actual_x1 = x1[:num_actual_nodes1]
        actual_x2 = x2[:num_actual_nodes2]
        combined_x = torch.cat([actual_x1, actual_x2], dim=-1)
        gate = torch.sigmoid(self.gate_layer(combined_x))
        
        mixed_actual = gate * actual_x1 + (1 - gate) * actual_x2
    ...
    def forward(self, data):
        ...
        for i in range(len(self.first_gat_convs)):
            ...
            mixed_actual = self.mix_actual_nodes(
                first_x_new, second_x_new, num_actual_nodes1, num_actual_nodes2,
            )
